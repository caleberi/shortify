import { applogger, ObjectId, passwordEncrypt, Router, User } from '../deps.ts';
import {
		...
		.put(
			...
			async (req, res, next) => {
				...
				try {
					...
					newUser.profileImageUrl = avatarUrl ||
						'https://api.adorable.io/avatars/400/9996761c47427399e4fc968899fa8843.png';
					newUser.updatedAt = new Date();

					const doc = await db.updateOne({ _id: new ObjectId(id) }, {
						$set: newUser,
					});
					return res.setStatus(200).json({
						status: 'SUCCESS',
			...
			try {
				...
				if (password !== confirmPassword) {
					return res.setStatus(400).json({
						status: 'FAILURE',
						message:
							'Password does not match for update to be made',
					});
				}
